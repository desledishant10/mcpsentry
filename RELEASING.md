...
ls -la dist/
python -m zipfile -l dist/mcpsentry-X.Y.Z-py3-none-any.whl | head -20

# Confirm entry points
unzip -p dist/mcpsentry-X.Y.Z-py3-none-any.whl mcpsentry-X.Y.Z.dist-info/entry_points.txt
```
...
```
[console_scripts]
mcpsentry-analyze = analyzer.__main__:main
mcpsentry-audit = harness.audit:main
mcpsentry-capture = harness.capture:main
mcpsentry-classify = classifier.__main__:main
mcpsentry-eval-calibration = calibration.eval:main
mcpsentry-lint-scenarios = analyzer.lint_scenarios:main
mcpsentry-scaffold-gt = calibration.scaffold:main
mcpsentry-test = harness.cli:main
```
...
If twine complains about the package name being taken on TestPyPI, that's because someone else already registered `mcpsentry` on the test instance. You can either rename for the test (e.g. `mcpsentry-rc1`) or proceed straight to real PyPI for the first publish since TestPyPI is best-effort.

### 4. Install + smoke-test from TestPyPI

```bash
# Fresh venv, isolated from your dev install
python -m venv /tmp/test-mcpsentry
source /tmp/test-mcpsentry/bin/activate

# Install from TestPyPI (--extra-index-url for transitive deps from real PyPI)
pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple/ mcpsentry

# Smoke tests
mcpsentry-audit --help
mcpsentry-analyze --help
mcpsentry-capture --help
...
Watch the output for the URL the package was published at. Should be `https://pypi.org/project/mcpsentry/X.Y.Z/`.
...
After the first PyPI publish, edit `README.md` to use `pip install mcpsentry` in the quickstart instead of `git clone`. Commit + push.

### 9. Verify install from real PyPI

```bash
python -m venv /tmp/verify-mcpsentry
source /tmp/verify-mcpsentry/bin/activate
pip install mcpsentry
mcpsentry-audit mcp-server-fetch
deactivate
rm -rf /tmp/verify-mcpsentry
```
...
- **PyPI name squat.** Verify `mcpsentry` is not already taken: `pip search mcpsentry` (deprecated) or visit https://pypi.org/project/mcpsentry/ in a browser. Should 404 until you publish.
